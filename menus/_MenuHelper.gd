...
#	scenes.NetMultiplayerMenu = load("res://menus/net_multiplayer/NetMultiplayerMenu.tscn")
#
#
...
#	menu.queue_free()
#
#func show_inventory(context = null, tab_filter = [], immediate_item_use = true, items_in_use = []):
#	var menu = scenes.InventoryMenu.instance()
#	menu.context = context
#	menu.tab_filter = tab_filter
#	menu.immediate_item_use = immediate_item_use
#	menu.items_in_use = items_in_use
#	add_child(menu)
#	var result = yield(menu.run_menu(), "completed")
#	menu.queue_free()
#	return result
...
#	return custom_confirm(message, "UI_BUTTON_YES", "UI_BUTTON_NO", default_index, initial_index)
#
#func custom_confirm(message: String, yes: String, no: String, default_index: int = 1, initial_index: int = 0):
#	GlobalMessageDialog.clear_state()
#	yield(GlobalMessageDialog.show_message(message, false, false), "completed")
#	var result = yield(GlobalMenuDialog.show_menu([yes, no], default_index, initial_index), "completed")
...
#	menu.queue_free()
#
#func show_net_multiplayer_menu(args: Dictionary = {}, pause = false):
#	var was_paused = SceneManager.paused
#	if pause:
#		WorldSystem.push_flags(WorldSystem.flags & WorldSystem.WorldFlags.SAVING_ENABLED)
#		SceneManager.paused = true
#	var menu = scenes.NetMultiplayerMenu.instance()
#	menu.args = args
#	add_child(menu)
#	yield(menu.run_menu(), "completed")
#	menu.queue_free()
#	if pause:
#		SceneManager.paused = was_paused
#		WorldSystem.pop_flags()
#
